...
mod tests {
    ...
    fn test_parse_porcelain_output() {
        ...
        assert!(
            status
                .files
                .contains(&(FileStatus::Modified, "modified.txt".to_string()))
        );
        assert!(
            status
                .files
                .contains(&(FileStatus::Added, "added.txt".to_string()))
        );
        assert!(
            status
                .files
                .contains(&(FileStatus::Deleted, "deleted.txt".to_string()))
        );
        assert!(
            status
                .files
                .contains(&(FileStatus::Untracked, "untracked.txt".to_string()))
        );
    ...
    fn test_parse_porcelain_output_edge_cases() {
        ...
        assert!(
            status
                .files
                .contains(&(FileStatus::Modified, "valid.txt".to_string()))
        );
        assert!(
            status
                .files
                .contains(&(FileStatus::Added, "another.txt".to_string()))
        );
    }
    ...
    fn test_parse_porcelain_all_status_types() {
        ...
        assert!(
            status
                .files
                .contains(&(FileStatus::Renamed, "renamed.txt".to_string()))
        );
        assert!(
            status
                .files
                .contains(&(FileStatus::Copied, "copied.txt".to_string()))
        );
        assert!(
            status
                .files
        ...
        assert!(
            status
                .files
                .contains(&(FileStatus::Ignored, "ignored.txt".to_string()))
        );
    }
    ...
    fn test_parse_porcelain_worktree_modifications() {
        ...
        assert!(
            status
                .files
                .contains(&(FileStatus::Modified, "worktree_modified.txt".to_string()))
        );
    }
    ...
    fn test_git_status_equality() {
        ...
        let files3 = vec![(FileStatus::Modified, "different.txt".to_string())];

        let status1 = GitStatus {
            files: files1.into_boxed_slice(),
        };
        let status2 = GitStatus {
            files: files2.into_boxed_slice(),
        };
        let status3 = GitStatus {
            files: files3.into_boxed_slice(),
        };
    ...
    fn test_git_status_clone() {
        let files = vec![(FileStatus::Modified, "file1.txt".to_string())];
        let status1 = GitStatus {
            files: files.into_boxed_slice(),
        };
    ...
    fn test_parse_porcelain_filenames_with_spaces() {
        ...
        assert!(
            status
                .files
                .contains(&(FileStatus::Modified, "file with spaces.txt".to_string()))
        );
        assert!(
            status
                .files
                .contains(&(FileStatus::Added, "another file.txt".to_string()))
        );
    }
    ...
    fn test_parse_porcelain_unicode_filenames() {
        ...
        assert!(
            status
                .files
                .contains(&(FileStatus::Modified, "测试文件.txt".to_string()))
        );
        assert!(
            status
                .files
                .contains(&(FileStatus::Added, "🚀rocket.txt".to_string()))
        );
    }
}
